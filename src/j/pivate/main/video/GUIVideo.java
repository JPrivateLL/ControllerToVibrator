...
public class GUIVideo extends JFrame {
	...
	private JProgressBar progressBar_0,progressBar_1,progressBar_2,progressBar_3;
	...
	public GUIVideo(EmbeddedMediaPlayerComponent component,VideoRumble thread) {
		super(version);
		...
		scrollBar.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				if(!updateSlider){
					videoThread.changeTime((float) scrollBar.getValue() / (float) scrollBar.getMaximum());
				}
			}
		});
		...
		scrollBar_1.addAdjustmentListener(new AdjustmentListener() {
			public void adjustmentValueChanged(AdjustmentEvent evt) {		
				...
				speed+=0.5f;
				videoThread.mediaPlayerComponent.getMediaPlayer().setRate(speed);
			}
		});
		...
		button_1.setToolTipText("Hold down left mouse and move mouse up/down/left/right to add vibrations");
		...
		button_2.setToolTipText("Hold down left mouse and move mouse up/down/left/right to add vibrations");
		...
		button_3.setToolTipText("Hold down left mouse and move mouse up/down/left/right to add vibrations");
		...
		button_4.setToolTipText("Hold down left mouse and move mouse up/down/left/right to add vibrations");
		...
		btnStartstop.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(final MouseEvent arg0) {
				if(videoThread.mediaPlayerComponent.getMediaPlayer().isPlaying()){
	...
	public void videoLoaded(){
		System.out.println(videoThread.mediaPlayerComponent.getMediaPlayer().getLength()/500);
		
		updateSlider = true;
		scrollBar.setMaximum((int) videoThread.mediaPlayerComponent.getMediaPlayer().getLength()/500);
		updateSlider = false;
	}
	...
	public void updateSlider() {
		//TODO
		updateSlider = true;
		scrollBar.setValue((int)(videoThread.mediaPlayerComponent.getMediaPlayer().getTime()/500));
		updateSlider = false;
	}
