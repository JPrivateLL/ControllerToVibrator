...
public class VideoRumble {
	...
	public void changeTime(float time) {
		mediaPlayerComponent.getMediaPlayer().setTime(
				(long) (mediaPlayerComponent.getMediaPlayer().getLength()
						* time));
	}
	...
	public void loadVideo() {
		...
		System.out.println(
				mediaPlayerComponent.getMediaPlayer().getLength() + " TEST");
		try {
		...
		for (int i = 0; i < Math
				.round(mediaPlayerComponent.getMediaPlayer().getLength()
						/ DEFAULTUPDATEDELAY)
				+ 50; i++) {
			String line = null;
			try {
			...
			public void run() {
				...
				while (running) {
					if (mediaPlayerComponent.getMediaPlayer().isPlaying()) {
						int time = (int) mediaPlayerComponent.getMediaPlayer()
								.getTime();
						int delta = time - timeOld;
						timeOld = time;

						if (time > mediaPlayerComponent.getMediaPlayer()
								.getLength()) {
							continue;
						}

						// sync delta is 0 most of the time because getTime() is
						// not accurate.
						if (delta != 0) {
							timesUpdatedBetween = 0;
						}

						// create extra frames to fix getTime() bug (showing the
						// same time for x seconds).
						int currentFrame = time / DEFAULTUPDATEDELAY
								+ timesUpdatedBetween;
						timesUpdatedBetween++;
						...
						for (int i = 0; i < vibrators.size(); i++) {
							vibrators.get(i)
									.rumble(vibration.get(currentFrame)[i]
											* GUIVideo.GUIStrength);
						}
						gui.changeStrength(
					...
					try {
						if (mediaPlayerComponent.getMediaPlayer()
								.getRate() != 0) {
							Thread.sleep((int) (DEFAULTUPDATEDELAY
									/ mediaPlayerComponent.getMediaPlayer()
											.getRate()));
						} else {
				...
				for (int i = 0; i < vibration.size(); i++) {
					try {
						writer.write(String.valueOf(
								vibration.get(i)[0] + " " + vibration.get(i)[1]
										+ " " + vibration.get(i)[2] + " "
										+ vibration.get(i)[3]));
						writer.newLine();
					} catch (final IOException e) {
